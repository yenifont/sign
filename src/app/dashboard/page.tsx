                                        ...
                                        <div className="mb-4 p-4 bg-white dark:bg-gray-800 rounded-lg">
                                            ...
                                            <div className="space-y-3">
                                                {user.authenticators.map((auth, index) => (
                                                    <div key={auth.id} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                                                        ...
                                                        <button
                                                            onClick={async () => {
                                                                if (confirm('Are you sure you want to remove this device?')) {
                                                                    try {
                                                                        const res = await fetch('/api/auth/authenticator/delete', {
                                                                            method: 'DELETE',
                                                                            headers: { 'Content-Type': 'application/json' },
                                                                            body: JSON.stringify({ credentialID: auth.id }),
                                                                        });
                                                                        if (res.ok) {
                                                                            checkAuth();
                                                                        } else {
